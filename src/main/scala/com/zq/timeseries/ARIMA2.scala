...
import com.zq.timeseries.MatrixUtil2.{toBreeze, dvBreezeToSpark}
...
object ARIMA2 {
  ...
  private def hannanRissanenInit(
    ...
    val arTerms1 = Lag2.lagMatTrimBoth(y, m, includeOriginal = false)
    val yTrunc = y.drop(m)
    ...
    val arTerms2 = Lag2.lagMatTrimBoth(yTrunc, p, includeOriginal = false).drop(math.max(q - p, 0))
    val errorTerms = Lag2.lagMatTrimBoth(errors, q, includeOriginal = false).drop(math.max(p - q, 0))
  ...
  private def findBestARMAModel(
    ...
    def fitTryBothStrategies(p: Int, q: Int, ts: Vector, intercept: Boolean): Try[ARIMAModel] = {
      Try(ARIMA2.fitModel(p, 0, q, diffedTs, intercept, method = "css-cgd")).orElse {
        Try(ARIMA2.fitModel(p, 0, q, diffedTs, intercept, method = "css-bobyqa"))
      }
    }
