...
class TimeSeriesKeyModel {
   ...
   * @param hiveContext
   */
//  def arimaModelKeyEvaluationSave(coefficients:RDD[(String,(String,(String,String,String),String,String))],forecastValue:RDD[(String,Vector)],hiveContext:HiveContext): Unit ={
//    /**把vector转置**/
...
//    val evaluationDf=hiveContext.createDataFrame(evaluationRddRow,schema)
//
...
//  }
   ...
   * @param hiveContext
   */
//  def holtWintersModelKeyEvaluationSave(sse:RDD[(String,Double)],forecastValue:RDD[(String,Vector)],hiveContext:HiveContext): Unit ={
//    /**把vector转置**/
//    val forecastRdd=forecastValue.map{
//      _ match{
//        case (key,forecast)=>forecast.toArray
...
//  }
   ...
   * @param hiveContext
   */
//  private def keySaveInHive(dateDataRdd:RDD[Row],hiveColumnName:List[String],hiveContext:HiveContext): Unit ={
//    //把dateData转换成dataframe
//    val schemaString=hiveColumnName(0)+" "+hiveColumnName(1)
//    val schema=StructType(schemaString.split(" ")
//      .map(fieldName=>StructField(fieldName,StringType,true)))
//    val dateDataDf=hiveContext.createDataFrame(dateDataRdd,schema)
//
//    //dateDataDf存进hive中
...
//  }
   ...
   * @param hiveContext
   */
//  def actualForcastDateKeySaveInHive(trainTsrdd:TimeSeriesRDD[String],forecastValue:RDD[(String,Vector)],predictedN:Int,startTime:String,endTime:String,sc:SparkContext,hiveColumnName:List[String],keyName:String,hiveContext: HiveContext): Unit ={
//    //在真实值后面追加预测值
...
//        keySaveInHive(dateDataRdd,hiveColumnName,hiveContext)
//      }
//    }
//  }
}