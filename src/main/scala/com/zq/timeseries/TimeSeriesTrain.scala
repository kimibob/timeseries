...
object TimeSeriesTrain {
  ...
  def timeChangeToDate(timeDataKeyDf:DataFrame,sqlContext: SQLContext,hiveColumnName:List[String],startTime:String,sc:SparkContext): DataFrame ={
      ...
      rowRDD=timeDataKeyDf.rdd.map{row=>
        row match{
          case Row(time,key,data)=>{
            ...
            Row(Timestamp.from(dt.toInstant), key.toString, data.toString.toInt)
          }
        }
      }
    ...
    val field=Seq(
      ...
      StructField(hiveColumnName(1), IntegerType, true)
    )
...
//      .select(hiveColumnName.head,hiveColumnName.tail:_*)
    ...
    //val csvfile = sparkSession.read.csv("data/timeSeriesDate.csv")
    val csvfile = sparkSession.read.csv("data/dataflow.csv")
    //csvfile.printSchema()
...
//      .select(hiveColumnName(0),hiveColumnName(0)+"Key",hiveColumnName(1))
    ...
    var dtIndex:UniformDateTimeIndex=DateTimeIndex.uniformFromInterval(
      ...
      ZonedDateTime.of(2018, 7, 3, 0, 0, 0, 0, zone),
      new DayFrequency(1))
    ...
    timeSeriesModel.actualForcastDateSaveInHive(filledTrainTsrdd,forecastValue,predictedN,startTime,endTime,sc,hiveColumnName,"dataflow",sqlContext)
  }
}